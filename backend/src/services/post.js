...
export const createNewPostService = (body, userId) => new Promise(async (resolve, reject) => {
    try {
        ...
        await db.Attribute.create({
            id: attributesId,
            price: +body.priceNumber <1 ? 1 `${1 * 1000000} đồng/tháng` : `${body.priceNumber} triệu/tháng`,
        ...
        await db.Province.findOrCreate({
            ...
            defaults: {
                ...
                value: body?.province?.includes('Thành phố') ? body?.province?.replace('Thành phố ', '') : generateCode(body?.province?.replace('Tỉnh ', ''))
            }
        });
